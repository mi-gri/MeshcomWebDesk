...
### 🤖 KI / AI – QSO-Zusammenfassung
- **Automatic QSO summaries** generated by an external AI API (OpenAI, Grok or Azure OpenAI)
- A 🤖 icon appears next to a callsign in the MH list and in the chat once the last contact is older than the configurable threshold (`ThresholdDays`, default 7 days) – or immediately when a summary already exists
- **Modal dialog**: clicking 🤖 opens a dialog with:
  - Last QSO time and message count (loaded from the MySQL database)
  - Stored summary text (or a „not yet generated" placeholder)
...
- Requires **MySQL database** to be configured (messages are read from the monitor table; summaries stored in `qso_summaries`)
- Configured in **Settings → 🤖 KI**; API key stored encrypted (`dp:` prefix via ASP.NET Core Data Protection)
- Enabled / disabled via `Ai.Enabled` – applies live without restart
